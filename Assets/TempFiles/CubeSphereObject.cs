...
public class CubeSphereObject
{
	...
	public byte[] ConstructMesh(int resolution, Texture2D heightMap)
    {
        float radius = 15;
        int chunks = 6 * (divisions + 1) * (divisions + 1);
		info = CubeSphere.GenerateFaces(resolution, divisions, radius);
		for (int i = 0; i < chunks; i++)
		{
            ...
            {
                LatLongDegr xy = GeoMaths.PointToLLR(vertices[v]/radius).ConvertToDegr();
